...
# The Converting-Copy Playbook

> An integrated playbook drawing the five fundamentals (`pat_copywriting-craft-fundamentals`) into an end-to-end copywriting routine. Pairs with the StoryBrand framework and supersedes ad-hoc copy review.
...
A 1-2 page document that captures the brand's voice in concrete terms: words we use / words we don't, sentence-rhythm preferences, tone-by-context (sales / product / support / crisis), the specific human voice the brand is modelled on (founder, CMO, named persona). Updated annually. Cards: `ins_handley-voice-as-moat-against-ai`, `ins_write-like-you-speak`.

### 2. Audience-language document (Moesta + Handley)

Verbatim buyer language collected from switch interviews, sales-call transcripts, and customer support tickets. The concrete words buyers use to describe their problems, the resistance language they offer when they don't buy, the metaphors they reach for. This is what you'll mirror in copy, not what marketing imagines they'd say. Card: `ins_jtbd-interviews-surface-customer-language`.

### 3. Three-level problem document (Miller)

For each major buyer persona, write the **external** problem (tangible), the **internal** problem (emotional), and the **philosophical** problem (why it's wrong this exists). All three layers used per piece, in proportions calibrated to the funnel stage. Card: `ins_miller-three-level-problem`.
...
Cards: `ins_schafer-pain-pleasure-polarity`, `ins_shleyner-headline-as-complete-argument`, `ins_20-percent-rule-headline`, `ins_miller-caveman-test`.
...
Cards: `ins_schafer-uncomfortable-honesty`, `ins_handley-self-laughter-as-quality-kpi`, `ins_gerhardt-founder-story-as-weapon`.
...
Cards: `ins_shleyner-clarity-beats-cleverness`, `ins_shleyner-vividness-creates-memorability`, `ins_shleyner-conciseness-as-respect`, `ins_so-what-as-content-diagnostic`, `ins_harland-scream-principle`, `ins_harland-benefits-over-pride`.
...
Card: `ins_harland-ideas-need-to-simmer`.
...
Card: `ins_gerhardt-distribution-beats-creation`.

## Format-specific overlays

Different surfaces require different emphasis:

- **Newsletter / direct email** → write to one subscriber (Handley `ins_handley-write-to-one-subscriber`); white space (Handley `ins_handley-white-space-as-oxygen`); voice-led.
...
- **Cold outreach** → accusation-audit opener (Voss `ins_voss-accusation-audit`); reader-centred ("you" not "we"); brevity dominates.
- **Social / micro-content** → testing-lab discipline (Gerhardt `ins_gerhardt-social-as-testing-lab`); validate ideas before scaling.
...
- `pat_copywriting-craft-fundamentals`, the underlying convergence pattern.
- `pat_buyer-mindset-not-product-features`, the related principle for sales conversations.
