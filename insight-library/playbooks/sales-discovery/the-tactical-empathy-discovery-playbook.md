...
domains: [sales-cs, research-discovery, pmm, leadership]
---

# The Tactical Empathy Discovery Playbook
...
The mirror surfaced the procurement / legal dynamic that direct questioning would not have. Card: `ins_voss-mirroring-forces-elaboration`.
...
Labelling acknowledges the emotion without conceding the position. The counterpart feels seen and the conversation can move from emotional rails to rational rails. Card: `ins_voss-tactical-empathy-labels`.
...
Calibrated questions reframe the conversation from positional ("I want X, you want Y") to cooperative ("how do we both get there"). Card: `ins_calibrated-questions-illusion-of-control`.
...
- **Moesta's switch interviews** (`ins_jtbd-interviews-surface-customer-language`) extend the same mechanisms across 8-15 buyer / non-buyer conversations to surface the verbatim resistance language that powers Dunford-grade positioning. Use the Voss tools inside each interview; aggregate across interviews for patterns.

- **Munger's incentive analysis** (`ins_incentives-as-master-switch`) is the analytical lens you apply to whatever surfaces. Once the rapport produces information, ask: what is actually rewarded in the counterpart's environment? Most surfaced behaviour resolves into rational choice given their actual payoff matrix.
...
Card: `ins_voss-black-swans-discovered-in-process`.
...
- `pat_rapport-surfaces-what-research-cannot` for the underlying convergence.
- `pat_status-quo-is-the-competitor` for the related pattern that positions switch interviews as B2B research method.
