...
- `ins_one-percent-price-eight-percent-profit`: 1% price increase yields 8-11% profit improvement; the leverage is enormous.
- `ins_simon-pricing-is-structural-org-failure`: <5% of Fortune 500 have a dedicated pricing department; structural under-investment.
- `ins_simon-discounting-most-dangerous-practice`: discounting is easy to start, nearly impossible to stop; the cumulative damage compounds permanently.
- `ins_simon-single-price-always-suboptimal`: WTP heterogeneity makes single-price strategies always leave money on the table.
- `ins_simon-formal-pricing-process`: the four-phase process (strategy / analysis / decision / implementation) and the Pricing Officer role.

**Madhavan Ramanujam**, the operational diagnosis.
- `ins_price-before-product`: 72% of innovations fail because companies design first and price later.
- `ins_ramanujam-three-wtp-questions`: the WTP-conversation method that surfaces price thresholds and demand cliffs.
- `ins_ramanujam-feature-shock`, `ins_ramanujam-minivation`, `ins_ramanujam-leaders-fillers-killers`: three product-pricing failure modes and the bundling framework that prevents them.
- `ins_ramanujam-ai-pricing-against-labor-budgets`: AI products should price against labour budgets (10× IT) and capture 25-50% of value, not the SaaS-typical 10%.

**David Skok**, the SaaS-economics expression.
- `ins_ltv-cac-ratio-and-cash-flow-trough`, `ins_skok-negative-churn`: pricing models with built-in expansion paths produce NRR > 100%, the structural marker of best-in-class SaaS.

**Alex Hormozi**, the offer-engineering layer.
- `ins_value-equation-grand-slam-offer`, `ins_premium-pricing-virtuous-cycle`, `ins_offer-and-market-as-highest-roi-lever`: pricing is the operational lever in offer design; market and offer beat funnel optimisation.
...
Cards listed under uses_cards above. See also `pat_market-and-offer-beat-funnel-optimisation` for the related upstream-vs-downstream pattern.
