---
id: pat_decision-quality-through-process-not-willpower
title: Decision quality at scale comes from process design, not from individual brilliance or harder thinking
...
domains: [leadership, strategy, founder-craft, research-discovery]
---
...
- **Daniel Kahneman — the bias inventory.**
  - System 1 / System 2: intuitive judgment is fast and lazy; deliberate reasoning is slow and rare. Treating intuition as truth is the first error in every hard call. (`ins_system1-system2-thinking`)
  - WYSIATI: confidence rises as data thins because the story is cleaner; the antidote is explicit unknown-enumeration. (`ins_wysiati-overconfidence`)
  - Noise: unwanted variance between judgments that should be identical, invisible without measurement, at least as damaging as bias. (`ins_noise-vs-bias-judgment-quality`)
  - Planning fallacy: every timeline is optimistic; the fix is reference-class forecasting (outside view). (`ins_planning-fallacy-launch-timelines`)
  - Anchoring: the first number sets the range; the negotiation is decided before it starts. (`ins_anchoring-pricing-negotiation`)
  - Loss aversion: a 2× emotional tax on switching that no value lift below 2× can clear. (`ins_loss-aversion-status-quo-bias`)

- **Charlie Munger — the multi-model operating system.**
  - Latticework: 80-90 mental models from multiple disciplines, internalized to run as an OS. (`ins_latticework-of-mental-models`, `ins_mental-models-as-os-not-library`)
  - Inversion: ask "what would guarantee failure?" before "how do I succeed?" (`ins_invert-always-invert`)
  - Incentives as master switch: explain puzzling behavior by identifying what is actually rewarded. (`ins_incentives-as-master-switch`)
  - Circle of competence: name the boundary; refuse to operate outside it. (`ins_circle-of-competence`)
  - Lollapalooza: when 3+ biases stack, single-model reasoning under-predicts the magnitude. (`ins_lollapalooza-effects`)
